...
<template>
    <div class="px-auto  w-full">
        ...
        <DataTable id="tabla" stripedRows class="p-datatable-sm" :scrollHeight="height" :value="elementos" v-model:filters="filters" dataKey="id" filterDisplay="menu" :loading="props.loading" :globalFilterFields="['code', 'name', 'email']" currentPageReportTemplate=" {first} al {last} de {totalRecords}" paginatorTemplate="FirstPageLink PrevPageLink PageLinks NextPageLink LastPageLink RowsPerPageDropdown" :paginator="true" :rows="10" :rowsPerPageOptions="[10, 25, 50, 100]">
                ...
                <div class="flex justify-between w-full h-8 mb-2">
                    <div class="flex space-x-4">
                        ...
                        <div class="relative flex rounded-md shadow-sm">
                            ...
                            <input type="search" class="block w-10/12 rounded-md border-0 py-4 pl-10 text-gray-900 ring-1  ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6" v-model="filters.global.value"  placeholder="Buscar..." />
                            </div>
                    </div>
                </div>
            </template>
            ...
            <Column header="Acciones" class="space-x-3" v-if="editItem || props.delete">
                <template #body="slotProps">
                    <!--BOTÓN EDITAR-->
                    <div class="whitespace-normal pl-4 pr-3 text-sm font-medium text-gray-900 sm:pl-6 lg:pl-8 flex space-x-2 ">
                                    ...
                                    <div class="mt-2 space-y-4 border border-gray-200 p-2 rounded-lg">
                                        <template v-for="header of headers">
                                            ...
                                            <div v-if="header.input !== false && header.type == 'multiple'">
                                                <label for="password" class="block capitalize text-left text-sm text-gray-900">{{header.header}}</label>
                                            <Dropdown  v-model="form.object[header.field]" :options="header.options[0]" filter optionLabel="name" placeholder="Selecccionar" class="w-full md:w-14rem">
                                            </Dropdown>
                                            </div>
                                        </template>
                                    </div>
