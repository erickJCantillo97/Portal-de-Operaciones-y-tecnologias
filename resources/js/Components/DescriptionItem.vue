  ...
  <span v-for="(item, index) of options" :key="index">
    ...
    <dl v-if="getFieldValue(item[0]) !== undefined && item[0] !== 'id'"
      ...
      <div class="flex justify-between py-3 text-sm font-medium">
        ...
        <dd
          v-if="item[0] == 'state' || item[0] == 'status' || item[0] == 'estado' || item[0] == 'Estado' && optionStatus[item[0]]"
          class="p-1 rounded-md " :class="optionStatus[item[0]].color">
          <i :class="optionStatus[item[0]].icon"></i>
