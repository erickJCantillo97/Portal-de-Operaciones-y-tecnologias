...
onMounted(() => {
    ...
    elementGantt = document.getElementById("containergantt");
})
...
const onEditTaskClick = () => {
    ...
    if (gantt.selectedRecord) {
        ...
        toast("Primero selecciona una tarea a editar");
    }
}
...
ganttConfig.project.transport.load.url= route('dataGantt', props.project)
ganttConfig.project.transport.sync.url= route('syncGantt', props.project)

</script>
<template>
    <AppLayout>
        <div class="h-[89vh] flex flex-col overflow-y-auto">
            <div class="rounded-t-lg h-8 flex space-x-0.5">
                ...
                <Button v-tooltip.bottom="'Pantalla completa'" icon="fa-solid fa-maximize" severity="help" raised
                    @click="maximize" />
                <span v-if="!error"
            ...
            <!-- <div id="containergantt" class="h-[79vh] text-xs shadow-lg overflow-y-auto" /> -->
            <BryntumGantt ref="gantt" class="h-full" v-bind="ganttConfig"/>
        </div>
    </AppLayout>
</template>
<style>
