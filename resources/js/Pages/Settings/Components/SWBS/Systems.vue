...
onMounted(() => {
    initFilters();
    loading.value = true;
    axios.get(route('system.index')).then(
        (res) => {
...
const editItem = (system) => {
    ...
    formData.name = system.name;
    formData.code = system.code;
    open.value = true;
};
...
const submit = () => {
    loading.value = true;
    if (formData.id == 0) {
        router.post(route('system.store'), formData, {
            preserveScroll: true,
            onSuccess: (res) => {
                open.value = false;
                toast('Sistema creado exitosamente', 'success');
            },
            onError: (errors) => {
    ...
    router.put(route('system.update', formData.id), formData, {
        preserveScroll: true,
        onSuccess: (res) => {
            open.value = false;
            toast('Systema actualizado exitosamente!', 'success');
        },
        onError: (errors) => {
            toast('¡Ups! Ha surgido un error', 'error');
        },
...
<template>
    <div class="px-auto  w-full">
        <DataTable id="tabla" stripedRows class="p-datatable-sm" :value="groups" scroll-height="300px"
            ...
            <Column field="constructive_group_id" header="Grupo constructivo"></Column>
                                ...
                                <div class="px-2 mt-2 text-center">
                                    ...
                                    <div class="p-2 mt-2 space-y-4 border border-gray-200 rounded-lg">
                                        <div class="col-span-1 py-2 md:col-span-4 p-fluid p-input-filled">
                                            <!-- <TextInput class="mt-2 text-left" type="string" label="Grupo constructivo"
                                                :placeholder="'Nombre del sistema'" v-model="formData.constructive_group_id"
                                            ...
                                            <TextInput class="mt-2 text-left" type="string" label="Nombre del sistema"
                                                :placeholder="'Nombre del sistema'" v-model="formData.name"
                                                :error="$page.props.errors.name">
                                            </TextInput>
                                        </div>
                                    </div>
