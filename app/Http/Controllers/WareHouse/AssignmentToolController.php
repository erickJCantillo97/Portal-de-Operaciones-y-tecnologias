...
class AssignmentToolController extends Controller
{
    ...
    {
        $assignmentsTool = DB::table('assigment_tools_view')->orderBy('assigment_date', 'DESC')->where('status', 'ASIGNADO')->get();
        $projects = Project::orderBy('created_at', 'DESC')->get();
        $warehouse = Warehouse::where('department', auth()->user()->oficina)->first()->id ?? 4;
        $tools = Tool::where('warehouse_id', $warehouse)->orderBy('category_id')->where('estado', '!=', 'ASIGNADO')->get();
